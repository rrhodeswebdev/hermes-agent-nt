...
// -----------------------------------------------------------------------------
//  A dark card-style HUD for the Hermes agent (status pill, P&L stat boxes,
//  daily-goal progress, armed-plan section, last decision, recent-decision
//  table) plus the agent's support/resistance (swing) lines.
//
//  DATA: polls the bridge's /panel.txt (structured key=value lines — no JSON
//  parsing in NinjaScript, same philosophy as /levels.txt). plan_* keys render
//  the ARMED PLAN section and appear once the analysis/execution-split bridge
//  is merged; until then the section shows "none armed".
//
//  THREADING / LOCKUP SAFETY (see E:\Coding\NinjaTrader\AGENTS.md):
//   * The background timer ONLY fetches + parses + caches an immutable snapshot
//     (overlap-gated via Interlocked). It makes NO ChartControl / Dispatcher /
//     Draw calls (gotcha #17 — dispatcher saturation locks the chart). NEVER add
//     a synchronous Dispatcher.Invoke anywhere reachable from a data thread —
//     that deadlocks against F5 reload / dialogs (dump-verified 2026-06-10).
//   * The card is drawn in OnRender with SharpDX only — no Draw.* there (gotcha
//     #13). The DirectWrite display list is rebuilt at most ~1×/s (or when a new
//     snapshot arrives); every other render pass just replays cached layouts.
//   * SharpDX brushes are device resources, handled per AGENTS.md §5b: EAGERLY
//     (re)created in OnRenderTargetChanged with a same-target skip
//     (ReferenceEquals + _dxInitialized), plus a self-heal re-init in OnRender if
//     NT8 swapped the target without firing the callback. Never created lazily
//     per draw — stale brushes on a disposed RT fault the UI thread (the
//     "chart-then-NT8" lockup).
//   * Every lifecycle override swallows its own exceptions with a bounded error
//     print (§5d — unhandled lifecycle exceptions count toward MaxRestarts).
//   * A volatile _terminated flag (set FIRST in State.Terminated) stops OnRender
//     from touching DX resources while Terminated disposes them.
//   * S/R lines still use Draw.HorizontalLine from OnBarUpdate, wall-clock
//     throttled (~5 Hz) so Calculate.OnEachTick cannot flood WPF.
//   * SharpDX here is 2.6.3 (NT8-shipped): no Mathematics.Interop Raw* types;
//     RectangleF takes width/height (NOT right/bottom) — see Ltrb().
//   * DRAG: the card is click-draggable. Preview mouse events are hooked ONCE on
//     the ChartControl via an ASYNC dispatcher hop (a sync Invoke here can ABBA-
//     deadlock with F5 reload's Clone — dump-verified 2026-06-10). Handlers run
//     on the UI thread: they check _terminated first, are fully try/caught
//     (gotcha #21), mutate only plain float fields, and repaint via
//     InvalidateVisual throttled to ~30 Hz. The drag offset is applied as a D2D
//     transform at replay time, so dragging never rebuilds the display list.
//     Double-click snaps the card back to its default corner. The offset
//     persists with the workspace via serialized CardOffsetX/Y properties.
//
//  KNOWN LIMIT: the LIVE pill's age only advances when the chart repaints (we
//  deliberately never poke the dispatcher from the timer). If the data feed dies
//  completely, the card freezes until the next repaint, which then shows STALE.
//
//  Put it on the MNQ trading chart (same timeframe as the bridge) so the S/R
//  lines sit at the right prices. Drag the card anywhere with the mouse
//  (double-click it to reset); the ▾/▸ glyph in its header folds it down to a
//  compact header + position/P&L strip. Compile inside NinjaTrader (F5).
// =============================================================================
    ...
    public class HermesDashboard : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    _stateErrorCount++;
                    Print("[HermesDashboard] OnStateChange error #" + _stateErrorCount
        ...
        private void OnStateChangeInner()
        {
            if (State == State.SetDefaults)
            {
                ...
                Description = "Hermes agent card (status / P&L / plan / decisions) + its S/R levels.";
            ...
            {
                // Earliest chance — but ChartControl can STILL be null here (notably on
                // the F5 clone-reload path), so OnRender retries until hooked.
            ...
            else if (State == State.Terminated)
            {
                ...
                // Synchronous cleanup so enable/disable cycles don't leak. Each step
                // guarded — Terminated must never throw (counts toward MaxRestarts).
                try
                {
        ...
        // =====================================================================
        //  S/R level lines — unchanged mechanism: Draw.* ONLY from the throttled
        //  OnBarUpdate (never from OnRender / the timer thread).
        // =====================================================================
        protected override void OnBarUpdate()
        {
            ...
            {
                // §5d: bounded — an unhandled OnBarUpdate exception counts toward MaxRestarts.
                if (_barErrorCount < 5)
                {
                    _barErrorCount++;
                    Print("[HermesDashboard] OnBarUpdate error #" + _barErrorCount + ": " + ex.Message);
                }
            }
        }
