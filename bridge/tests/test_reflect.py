...
def test_reflect_profile_replace_becomes_proposal(tmp_path, monkeypatch):
    cfg, learned, journal, r = _reflector(tmp_path)
    learned.set_profile("user rules")
    reply = json.dumps({"is_error": False, "structured_output": {
        "lessons": [], "profile_replace": "rewritten profile"}})
    monkeypatch.setattr("hermes_bridge.reflect.run_claude_oneshot", lambda *a, **k: reply)
    applied = r.reflect_on_close(_trade(), [])
    assert applied["profile"] == 1
    assert learned.profile() == "user rules"  # live file untouched — proposal only
    proposed = Path(cfg.learning.learned_dir) / "trader-profile.proposed.md"
    assert "rewritten profile" in proposed.read_text(encoding="utf-8")
