...
from .config import BridgeConfig, load_config, timeframe_seconds
...
class AppState:
    def __init__(self, config: BridgeConfig) -> None:
        self.cfg = config
        self.entry_freshness_s = (config.execution.entry_freshness_s
                                  or timeframe_seconds(config.instrument.timeframe))
    ...
    def ingest_bar(request: Request, payload: BarIngest) -> Decision:
        ...
        with st.lock:
            ...
            if cmd is not None and is_stale_entry(cmd.action, elapsed, st.entry_freshness_s):
                d = Decision(action=Action.WAIT, rationale=f"stale_entry:{elapsed:.0f}s")
                cmd = None
            if cmd is not None:
                st.queue.push(cmd)
