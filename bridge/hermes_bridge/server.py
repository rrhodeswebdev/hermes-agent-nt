...
def should_drop_stale(cmd: OrderCommand, elapsed_s: float, budget_s: float) -> bool:
    """Market entries chase price when late; a resting limit cannot — never drop it."""
    if cmd.limit_price is not None:
        return False
    return is_stale_entry(cmd.action, elapsed_s, budget_s)
...
def create_app(config: BridgeConfig | None = None) -> FastAPI:
    ...
    def _dashboard_payload(st: AppState) -> dict:
        ...
        return {
            ...
            # Armed-plan view; render_panel turns this into the card's plan_* keys.
            "plan": st.engine.plan_snapshot(),
    ...
    def ingest_bar(request: Request, payload: BarIngest) -> Decision:
        ...
        with st.lock:
            ...
            if cmd is not None and should_drop_stale(cmd, elapsed, st.entry_freshness_s):
                st.stale_drops += 1
