...
class ExecutionConfig(BaseModel):
    ...
    # Armed-plan split: the agent may ARM an entry plan (zone) that rests as a NT8
    # limit order — zero decision latency at the trigger, and resting limits are
    # exempt from the freshness drop (they cannot chase). KEEP FALSE until the
    # strategy build that understands limit_price is compiled in NinjaTrader — an
    # old build would execute the arm as an immediate MARKET order.
    armed_plans: bool = False
